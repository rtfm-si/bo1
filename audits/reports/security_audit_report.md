...
1. **H5 - Remove Docker default password**
   - Remove `:-bo1_dev_password` fallback from `docker-compose.yml`
   - Require explicit `POSTGRES_PASSWORD` env var

2. **H1 - Enforce DEBUG=false in production**
   - Add startup validation: reject `DEBUG=true` when `ENVIRONMENT=production`
   - Already have `require_production_auth()` - add debug mode check

3. **H2 - Tighten CSP**
   - Remove `'unsafe-eval'` if possible (may require SvelteKit config changes)
   - Add `nonce` for inline scripts instead of `'unsafe-inline'`

4. **H3 - Strengthen prompt injection defense**
   - Already using `check_for_injection()` - ensure coverage on all user inputs
   - Add input length limits (max 10,000 chars for problem_statement)

### MEDIUM PRIORITY

5. **M1 - Redact PII in logs**
   - Hash or mask email addresses before logging

6. **M4 - Redis authentication**
   - Enable `requirepass` in Redis config for all environments

7. **M6 - Input validation limits**
   - Add `max_length=10000` to `CreateSessionRequest.problem_statement`

### LOW PRIORITY

8. **L1 - Remove console.log**
   - Remove debug logging from `+page.server.ts` files

9. **L3 - Audit @html usage**
   - Verify all `@html` uses are sanitized with DOMPurify
