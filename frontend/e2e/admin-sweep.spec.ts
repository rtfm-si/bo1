...
const ADMIN_PAGES = [
  ...
  { path: "/admin/waitlist", name: "Waitlist", heading: /Waitlist/i },
  {
    ...
    name: "Whitelist",
    heading: /Whitelist|Beta Whitelist/i,
  },
  { path: "/admin/promotions", name: "Promotions", heading: /Promotions/i },
  {
    path: "/admin/feedback",
    name: "Feedback",
    heading: /Feedback|User Feedback/i,
  },
  {
    path: "/admin/metrics",
    name: "Metrics",
    heading: /Metrics|Usage Metrics/i,
  },
  { path: "/admin/embeddings", name: "Embeddings", heading: /Embeddings/i },
  {
    path: "/admin/kill-history",
    name: "Kill History",
    heading: /Kill History/i,
  },
  {
    path: "/admin/ops",
    name: "AI Ops",
    heading: /AI Ops|Self-Healing|Operations/i,
  },
  {
    path: "/admin/alerts/settings",
    name: "Alert Settings",
    heading: /Alert|Settings/i,
  },
  {
...
const mockWhitelist = {
  entries: [
    { id: "1", email: "beta@test.com", created_at: new Date().toISOString() },
  ],
  total: 1,
};
...
test.describe("Admin Pages Sweep", () => {
  ...
  for (const adminPage of ADMIN_PAGES) {
    test(`${adminPage.name} page loads and displays correctly`, async ({
      page,
    }) => {
      // Navigate to page
      ...
      if (page.url().includes("/login")) {
        addFinding(
          adminPage.name,
          "error",
          "Redirected to login - auth mock may have failed",
        );
        test.skip();
        return;
      }
      ...
      // Check page heading
      const heading = page
        .getByRole("heading", { name: adminPage.heading })
        .first();
      const headingVisible = await heading.isVisible().catch(() => false);

      if (!headingVisible) {
        // Try alternative: check for any heading containing the name
        const altHeading = page
          .locator("h1, h2")
          .filter({ hasText: new RegExp(adminPage.name, "i") })
          .first();
        ...
        if (!altVisible) {
          addFinding(
            adminPage.name,
            "warning",
            `Expected heading "${adminPage.heading}" not found`,
          );
        }
      }
      ...
      if (hasErrorAlert) {
        ...
        addFinding(
          adminPage.name,
          "warning",
          `Error alert displayed: ${errorText}`,
        );
      }
      ...
      if (spinnerVisible) {
        addFinding(
          adminPage.name,
          "warning",
          "Loading spinner still visible after 2s",
        );
      }
  ...
  test("Admin dashboard navigation links work", async ({ page }) => {
    ...
    // Check if admin content loaded (skip if auth mocking failed)
    const visibleLinks = await page.locator('a[href^="/admin/"]').count();
    if (visibleLinks === 0) {
      // Admin auth mocking not working in this environment
      test.skip();
      return;
    }
    ...
    for (const link of navLinks) {
      ...
      if (!linkVisible) {
        addFinding(
          "Dashboard",
          "warning",
          `Navigation link to ${link.name} (${link.href}) not visible`,
        );
      }
    }
  ...
  test("Admin sessions page buttons work", async ({ page }) => {
    ...
    // Check refresh button - skip if admin auth mocking failed
    ...
    const checkboxVisible = await autoRefreshCheckbox
      .isVisible()
      .catch(() => false);
    if (checkboxVisible) {
