<script lang="ts">
	...
	const CONFIG_LABELS: Record<string, { label: string; description: string; dangerWhenOff: boolean }> = {
		prompt_injection_block_suspicious: {
			label: 'Block Prompt Injection',
			description: 'When enabled, blocks requests containing suspicious prompt injection patterns. Disable only in emergencies.',
			dangerWhenOff: true
		}
	};
