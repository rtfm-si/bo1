<script lang="ts">
	...
	// Clarification required event (for pre-deliberation Q&A)
	const clarificationRequiredEvent = $derived(
		events.find(e => e.event_type === 'clarification_required')
	);

	// Track if clarification has been answered
	let clarificationAnswers = $state<Record<string, string>>({});
	let isSubmittingClarification = $state(false);
	let clarificationError = $state<string | null>(null);

	// Check if we need to show clarification UI
	const clarificationQuestions = $derived(
		clarificationRequiredEvent?.data?.questions as Array<{ question: string; reason?: string; priority?: string }> | undefined
	);
	const needsClarification = $derived(
		clarificationRequiredEvent !== undefined &&
		session?.status === 'active' &&
		clarificationQuestions !== undefined &&
		Object.keys(clarificationAnswers).length < clarificationQuestions.length
	);
	...
	async function startEventStream() {
		...
		const eventTypes = [
			...
			'clarification_required', // Pre-deliberation clarifying questions
	...
	async function handleSubmitClarifications() {
		if (!clarificationRequiredEvent) return;
		...
		try {
			...
			const response = await fetch(`${env.PUBLIC_API_URL}/api/v1/sessions/${sessionId}/clarifications`, {
				method: 'POST',
				headers: {
			...
			clarificationError = err instanceof Error ? err.message : 'Failed to submit answers';
		} finally {
			isSubmittingClarification = false;
		}
	}

	// Handle skipping clarification questions
	async function handleSkipClarifications() {
		...
		try {
			...
			clarificationError = err instanceof Error ? err.message : 'Failed to skip questions';
		} finally {
			isSubmittingClarification = false;
		}
	}
...
<div class="min-h-screen bg-neutral-50 dark:bg-neutral-900">
	...
	<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		...
		<!-- Clarification Questions (Pre-Deliberation Q&A) -->
		{#if needsClarification && clarificationRequiredEvent}
			<div class="mb-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-6" transition:fade>
				<div class="flex items-start gap-4">
					...
					<div class="flex-1 min-w-0">
						...
						<p class="text-sm text-amber-700 dark:text-amber-300 mb-4">
							{clarificationRequiredEvent.data.reason || 'Answering these questions will help our experts provide better recommendations.'}
						</p>

						<div class="space-y-4">
							{#each clarificationQuestions ?? [] as question, index}
								<div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-amber-100 dark:border-amber-800">
									<label for="clarification-{index}" class="block text-sm font-medium text-slate-900 dark:text-white mb-2">
										{question.question}
									...
									<textarea
										...
										rows="2"
										bind:value={clarificationAnswers[question.question]}
						...
						{#if clarificationError}
							<div class="mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
								<p class="text-sm text-red-700 dark:text-red-300">{clarificationError}</p>
							</div>
						{/if}
