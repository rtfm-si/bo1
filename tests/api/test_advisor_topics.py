...
class TestRepeatedTopicsEndpoint:
    """Tests for GET /api/v1/advisor/repeated-topics."""

    def test_repeated_topics_endpoint_auth(self, unauthenticated_client):
        """Endpoint requires authentication."""
        response = unauthenticated_client.get("/api/v1/advisor/repeated-topics")
    ...
    def test_repeated_topics_returns_topics(self, client):
        ...
        with (
            patch("backend.api.advisor.get_mentor_conversation_repo") as mock_repo,
            ...
            response = client.get("/api/v1/advisor/repeated-topics")
    ...
    def test_repeated_topics_threshold_validation(self, client):
        ...
        # Too low
        response = client.get("/api/v1/advisor/repeated-topics", params={"threshold": 0.5})
        assert response.status_code == 422

        # Too high
        response = client.get("/api/v1/advisor/repeated-topics", params={"threshold": 0.99})
        assert response.status_code == 422

    def test_repeated_topics_min_occurrences_validation(self, client):
        """min_occurrences must be between 2 and 10."""
        # Too low
        response = client.get("/api/v1/advisor/repeated-topics", params={"min_occurrences": 1})
        assert response.status_code == 422

        # Too high
        response = client.get("/api/v1/advisor/repeated-topics", params={"min_occurrences": 15})
        assert response.status_code == 422

    def test_repeated_topics_days_validation(self, client):
        """days must be between 7 and 90."""
        # Too low
        response = client.get("/api/v1/advisor/repeated-topics", params={"days": 3})
        assert response.status_code == 422

        # Too high
        response = client.get("/api/v1/advisor/repeated-topics", params={"days": 120})
        assert response.status_code == 422
...
class TestRepeatedTopicsCache:
    ...
    def test_repeated_topics_cache_hit(self, client):
        ...
        with (
            ...
            # First call
            response1 = client.get("/api/v1/advisor/repeated-topics")
            assert response1.status_code == 200

            # Second call - still works (caching at detector level)
            response2 = client.get("/api/v1/advisor/repeated-topics")
