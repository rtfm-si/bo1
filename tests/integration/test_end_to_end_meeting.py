...
@pytest.fixture
def redis_manager_fixture() -> RedisManager:
...
@pytest.fixture
def event_publisher(redis_manager_fixture: RedisManager) -> EventPublisher:
    """Create event publisher for tracking events."""
    return EventPublisher(redis_manager_fixture.redis)
    ...
    def collect_event(self, timeout: float = 5.0) -> dict[str, Any] | None:
        ...
        if msg and msg["type"] == "message":
            try:
                ...
                logger.warning(f"Failed to parse event: {msg}")
        return None

    def collect_events_until(self, event_type: str, timeout: float = 30.0) -> list[dict[str, Any]]:
...
async def test_complete_meeting_lifecycle(
    ...
    redis_manager_fixture: RedisManager,
    ...
    try:
        ...
        event_capture = EventCapture(redis_manager_fixture.redis, test_session_id)
        event_capture.start()
        ...
        logger.info(f"✓ Decomposition: {len(final_state['problem'].sub_problems)} sub-problems")
...
async def test_meeting_with_multiple_subproblems(
    ...
    try:
        ...
        logger.info(
            f"✓ Multi-sub-problem test: {num_subproblems} problems, {len(results)} completed"
        )
...
async def test_meeting_convergence_triggers(
    ...
    try:
        ...
        logger.info(f"Convergence test: stopped at round {final_state['round_number']}/10")
